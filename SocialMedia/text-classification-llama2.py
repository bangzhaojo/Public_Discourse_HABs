...
def generate_batch_responses(text_inputs, model, tokenizer, template, batch_size=8):
    ...
    for i in tqdm(range(0, len(text_inputs), batch_size), desc="Processing"):
        ...
        if (i // batch_size) % 100 == 0:
            filename = f"/shared/3/projects/bangzhao/lake-erie/llama2-{i}.json"
...
with open('/shared/3/projects/bangzhao/lake-erie/data_cleaned/all_RC.json', 'r') as json_file:
...
filename = "/shared/3/projects/bangzhao/lake-erie/llama2.json"
with open(filename, "w") as json_file:
    json.dump(responses, json_file)
